...
const FlightList = ({ handleClick }) => {
  return (
    <div>
      <div>
        <h2>Flight List</h2>
        <div className="border-2 border-gray-75 my-1 grid grid-rows-2 rounded-lg grid-cols-12 mx-80 justify-center">
          ...
          <button className="row-start-1 row-span-2 col-start-11 bg-blue-200 rounded-full w-1/2 h-1/2 justify-self-center self-center">Detail</button>
          <button onClick={() => handleClick()} className="row-start-1 row-span-2 col-start-12 bg-blue-200 rounded-full w-1/2 h-1/2 justify-self-center self-center">Select</button>
        </div>
      </div>
    </div>
  );
